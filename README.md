...
- **Phase 2（LLM品質とプロバイダ切替）**: 着手中
  ...
  - クラウドAIの最小実装を追加。設定画面でモデル名とAPIキーを管理し、APIキーはAndroid Keystore由来の鍵で暗号化して保存。クラウドモードでは OpenAI Chat Completions API に接続し、失敗時は簡易モードへフォールバック
...
5. （任意）「設定」でクラウドAIのモデル名とAPIキーを保存し、クラウドモードへ切替
   - APIキー未保存時はクラウドモードを選べず、通信失敗時も簡易モードへフォールバック
...
- 設計: MVVM + 単方向データフロー
- LLM: `LlmProvider` 抽象（既定Stub、設定でオンデバイス MediaPipe LLM Inference / OpenAI Chat Completions API に切替）
